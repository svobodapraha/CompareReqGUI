         ...
         if(asOldReqID == asNewReqID)
         {
             ...
             foreach (QString asHeader, lstNewOldHeader)
             {
                ...
                if(
                     ...
                     (
                       (
                          (getCellValue(newRow, iNewColObjectType, newReqDoc, true) == "Functional Requirement") ||
                          (getCellValue(oldRow, iOldColObjectType, oldReqDoc, true) == "Functional Requirement") ||
                ...
                //different
                {
                   ...
                   //another column from the same request..
                   if(boSameReq)
                   {
                       ...
                       {
                           ui->tableWidget_Changes->item((ui->tableWidget_Changes->rowCount()-1)-1, col_infotable_requirement)->setBackgroundColor(QColor(Qt::lightGray));
                           ui->tableWidget_Changes->item((ui->tableWidget_Changes->rowCount()-1)-1, col_infotable_req_short)->setBackgroundColor(QColor(Qt::lightGray));
                       }
                   }
                   boSameReq = true;
                   ...
                   //yellow if not function requiments
                   if(
                         (getCellValue(newRow, iNewColObjectType, newReqDoc, true) != "Functional Requirement") &&
                         (getCellValue(oldRow, iOldColObjectType, oldReqDoc, true) != "Functional Requirement")
                     )
